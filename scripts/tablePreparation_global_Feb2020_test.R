...
lsAll <- lapply(vecLevelFinal,function(reg){
  ...
  lsAggregate <- lapply(c(1961,1971,1981,1991,2001),function(yearStart){
    ...
    if(sumLevel==10){
      ...
      dfSummary$meanCroplandStability <- mean(dfCroplandLevel$croplandArea,na.rm=T)/sd(dfCroplandLevel$croplandArea,na.rm=T)
...
dfAll <- dfAll[,c("Country","timePeriod",
                  ...
                  "yieldAsynchrony","productionAsynchrony","meanCroplandStability",
                  "richness","diversity","nitrogen","nitrogen_ha","irrigation","irrigation_share",
                  "instabilityTemp","instabilityPrec","warfare")]
write.csv(dfAll, "datasetsDerived/dataFinal_global_Feb2020_test.csv",row.names=F)
