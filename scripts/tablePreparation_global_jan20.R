...
################ DATA PREPARATION
## production data
prod.dat <- read.csv("datasets/agriculturalProduction_global_withFlags.csv")
nut.info <- read.csv("C:/Users/egli/Desktop/cropNutrients_DRenard2019.csv", sep=";") #nutrient info
dim(prod.dat)
dim(nut.info)

#### Remove unecessary Elements from the database
prod.dat <- droplevels(prod.dat[!prod.dat$Element == "Yield", ] ) # yield
prod.dat <- droplevels(prod.dat[!prod.dat$Item == "Mushrooms and truffles",]) # not plant but fungi

########################### 1) Shape dataframe with flag info

#### Shape flag data 
prod.dat.flag <- cbind(prod.dat[,c(2,4,6)],prod.dat[,grepl("F",names(prod.dat))])

# From wide to long format, Year in one column
prod.dat.flag <- reshape(prod.dat.flag[,1:length(prod.dat.flag)], varying = names(prod.dat.flag)[4:length(prod.dat.flag)], v.names = "Flag", timevar = "Year", 
                         times = names(prod.dat.flag)[4:length(prod.dat.flag)], direction = "long")
row.names (prod.dat.flag) = NULL
prod.dat.flag <- prod.dat.flag[,-6]
prod.dat.flag$Year <- gsub('Y|F',"",prod.dat.flag$Year)
levels(prod.dat.flag$Flag)[levels(prod.dat.flag$Flag)==""] <- "Of"

# From long to wide format
prod.dat.flag <- dcast(prod.dat.flag, Area+Item+Year ~ Element, value.var = "Flag")
colnames(prod.dat.flag)[4:5] <-c("F.Area.Harvested","F.Production") 
summary(prod.dat.flag)

##### Shape data 
prod.dat.dat <- prod.dat[,-c(1,3,5,7,grep("F$",colnames(prod.dat)))]

# From wide to long format, Year in one column
prod.dat.dat  <- reshape(prod.dat.dat[,c(1:length(prod.dat.dat))], varying = names(prod.dat.dat)[4:length(prod.dat.dat)], v.names = "Value", timevar = "Year", 
                         times = names(prod.dat.dat)[4:length(prod.dat.dat)], direction = "long")
row.names (prod.dat.dat) = NULL
prod.dat.dat <- prod.dat.dat[,-6]
prod.dat.dat$Year <- gsub('Y',"",prod.dat.dat$Year)

# From long to wide format
prod.dat.dat <- dcast(prod.dat.dat, Area+Item+Year ~Element, value.var = "Value")
colnames(prod.dat.dat)[4] <-c("Area.Harvested") 

##### Merging data sets 
dat <- prod.dat.dat %>%
  left_join(prod.dat.flag, by =c('Area','Item','Year'))

summary(dat)
unique(droplevels(as.factor(dat$Item)))

#### Setting NA for harvested data for which there is no production data, 
#### and the same for production data for which there is no harvested data. Doing this to avoid messing with aggregated yields calculations
dat$Area.Harvested[is.na(dat$Production)] <- NA
dat$Production[is.na(dat$Area.Harvested)] <- NA
dat <- dat[!is.na(dat$Area.Harvested) & !is.na(dat$Production),]


########################### 2) Compute species production from crop nutrient info

colnames(nut.info)[1]<-"Item" # rename to match with dat

#### Add nutrient info
dat<-dat %>%left_join(nut.info[,c(1,5,6)], by = 'Item')
dat$Item = as.factor(dat$Item)

#### Removing crops with no nutrient data
dat <- dat[!is.na(dat$kcal_100g),]
dim(dat)
unique(dat$Item)#142

#### Calculate production in kcal and species yield per year
dat$Production.Kcal.millions <- dat$Production* 1000 *dat$kcal_100g * 10 / 1e6
dat$sp.yield.Kcal <-dat$Production.Kcal.millions / dat$Area.Harvested
summary(dat)# error due to null area harvested

#### remove null area harvested and null production
dat<-droplevels(dat[-which(dat$Area.Harvested==0),])
dat<-droplevels(dat[-which(dat$Production==0),])

summary(dat)# important outliers in sp.yield kcal
dat[dat$sp.yield.Kcal==max(dat$sp.yield.Kcal),]
unique(dat$Item)#142

########################### 3) Removing nations with no ISO3 code ("world", "Africa"...)
dat$ISO3 <- countrycode(dat$Area, 'country.name', 'iso3c')
dat$ISO3[dat$Area=="Eswatini"] = "SWZ"

dat<- droplevels(subset(dat, !(dat$ISO3 == "NA")))
unique(dat$ISO3)
...
# Country already targeted in Renard & Tilman : North Korea (PRK), Guinea (GIN), Kenya (KEN), 
# Mozambique (MOZ), Zambia (ZMB), Ireland (IRL), Netherland (NLD), New Zealand (NZL) and Egypt (EGY)

datX<- droplevels(subset(dat, !(dat$ISO3 %in% c("RUS", "SUN" ,"CSK", "CZE", "SVK", "SCG", "SRB", "MNE",
                                                "SSD", "SDN", "SFR", "PRK", "GIN,", "KEN","MOZ","ZMB","IRL","NLD", "NZL", "EGY","ETH"))))
dim(datX)
dat=datX

# China : continental or country
summary(dat[dat$ISO3=="CHN",])
dat<-droplevels(dat[!dat$Area=="China, mainland",])
dat[dat$sp.yield.Kcal==max(dat$sp.yield.Kcal),]
dat[dat$sp.yield.Kcal==max(dat$sp.yield.Kcal),]
length(unique(dat$ISO3))#197

########################### 4) Decade information

#### Add information by decade : from 1968 to 2018 or from 61 to 10
dat$P=0
dat[dat$Year%in%c(1968:1977),]$P = 1
dat[dat$Year%in%c(1978:1987),]$P = 2
dat[dat$Year%in%c(1988:1997),]$P = 3
dat[dat$Year%in%c(1998:2007),]$P = 4
dat[dat$Year%in%c(2008:2017),]$P = 5

# dat$P=1
# dat[dat$Year%in%c(1971:1980),]$P = 2
# dat[dat$Year%in%c(1981:1990),]$P = 3
# dat[dat$Year%in%c(1991:2000),]$P = 4
# dat[dat$Year%in%c(2001:2010),]$P = 5
# dat[dat$Year%in%c(2011:2017),]$P = 6

dat <- dat[order(dat$Area, dat$Year),]
dat[,c(1:3,12,13)]<-data.frame(apply(dat[,c(1:3,12,13)], 2, function(x){ as.factor(x)}))

#### Remove Spices, nes and onions dry and outliers in species yield
dat<-droplevels(dat[!dat$Item%in%c("Spices, nes","Onions, dry"),])
unique(dat$Item)#140
dat<-droplevels(dat[!dat$sp.yield.Kcal>1000,])
dat[dat$sp.yield.Kcal>100,]

########################################################################################################################
########### compute diversity, stability and community wide synchrony based on species present in each year of a period 
########################################################################################################################

########################### 1) Count number of year a species is present per decade

##### Species by country and decade
datX<-dat
datX$N<-NA
dat.count.sp<-split(datX,list(droplevels(datX$ISO3), droplevels(datX$P), droplevels(datX$Item)), drop=T)

##### count number of year a species is cultivated 

for (i in 1:length(dat.count.sp))
{
  print(i)
  dat.count.sp[[i]]$N<-nrow(dat.count.sp[[i]])
}

##### dataframe
dat.count.spX<-ldply(dat.count.sp)
dat.count.spX<-dat.count.spX[,-1]
summary(dat.count.spX)

##### remove P6
# dat.count.spX<-droplevels(dat.count.spX[!dat.count.spX$P=="6",])

##### Remove species present during 10 years
dat.count.spX<-droplevels(dat.count.spX[dat.count.spX$N==10,])
dim(dat.count.spX)
dat.count.sp<-dat.count.spX

########################### 2) Compute crop diversity aggregated by decade

#### Reshape data frame
div <- reshape(dat.count.sp[,c(12,13,2:4)], timevar = "Item", idvar = c("P","ISO3","Year"), direction = "wide")
div[100:110,1:10]

#### replace NA with 0 for vegan analysis
div[is.na(div)] <- 0

#### check every line as at least one information
y<-rowSums(div[,4:141],na.rm=T)
summary(y)

#### aggregate by country by decade
div.dec<-aggregate(div[,4:141], list (ISO3 = div$ISO3,P=div$P),mean)

#### Total species richness, number of effective species 
div.dec$rich<-specnumber(div.dec[,3:140])
div.dec$div<-exp(diversity(div.dec[,3:140]))

hist(div.dec$rich)
hist(div.dec$div)


########################### 4) Keep community with at least 10 species  # for synchrony to be pertinent but can be lowered 
##### Put number of species in species annual dataframe
dat.ann.sp <- dat.count.sp %>% 
  left_join(div.dec[,c(1,2,141,142)])

##### select communities
dat.ann.sp10<-droplevels(dat.ann.sp[dat.ann.sp$rich >= 10,])

##### split by community
#dat.ann.sp10$Year<-as.numeric(dat.ann.sp10$Year)
dat.com<-split(dat.ann.sp10,list(dat.ann.sp10$ISO3, dat.ann.sp10$P), drop=T)# 900
dat.com<-lapply(dat.com, function (x) droplevels(x))

##### Keep country with data available for the 10 years of a decade
nyear<-unlist(lapply(dat.com, function(x) length(unique(levels(x$Year)))))
dat.comX<- dat.com[nyear==10]
dat.com<-dat.comX #768


########################### 5) stability and synchrony 

##### Dataframe
data_tot<-data.frame(matrix(NA,nrow=length(dat.com),ncol=4))
##### Boucle

for (i in 1:length(dat.com)) {
  print(i)
  #### community i
  dat<-dat.com[[i]]
  ...
  dat2 <- dat %>% 
    group_by(Year)%>%
    summarise(T.Area.Harvested=sum(Area.Harvested, na.rm=TRUE),
              T.Production.kcal.millions = sum(Production.Kcal.millions, na.rm=TRUE))
  
  ## total annual yield
  dat2$T.ann.yield.kcal <- (dat2$T.Production.kcal.millions/dat2$T.Area.Harvested)
  
  ## Detrended annual yield by nation by decade
  dat2$Year<-as.numeric(dat2$Year)
  dat2$dtrd.Tyield.kcal <- residuals(loess(T.ann.yield.kcal~Year, dat2 ))
  dat2$dtrd.Tproduction.kcal <- residuals(loess(T.Production.kcal.millions~Year, dat2 ))
  
  ## Detrended Stability
  data_tot[i,1]<- mean(dat2$T.ann.yield.kcal, na.rm=TRUE) / sd (dat2$dtrd.Tyield.kcal, na.rm=TRUE)
  data_tot[i,2]<- mean(dat2$T.Production.kcal.millions, na.rm=TRUE) / sd (dat2$dtrd.Tproduction.kcal, na.rm=TRUE)
  
  ##### synchrony
  # detrended species annual yield
  dat$Year= as.numeric(dat$Year)
  
  for (j in unique(dat$Item))
  {
    dat$dtrd.yield[dat$Item==j] = residuals(loess(sp.yield.Kcal ~ Year,dat[dat$Item==j,]))
    dat$dtrd.production[dat$Item==j] = residuals(loess(Production.Kcal.millions ~ Year,dat[dat$Item==j,]))
    
  }
  
  data_tot[i,3]<-1-synchrony(dat,time.var="Year",species.var="Item",abundance.var="dtrd.yield") # from codyn library
  data_tot[i,4]<-1-synchrony(dat,time.var="Year",species.var="Item",abundance.var="dtrd.production") # from codyn library
}

head(data_tot)
colnames(data_tot)<-c("stability","stabilityProduction","asynchronyYield","asynchronyProduction")
data_tot$ISO3<-as.factor(substr(names(dat.com),1,3))
data_tot$P<-as.factor(substr(names(dat.com),5,5))

head(data_tot)

mod1 <- lm(stability~asynchronyYield,data_tot)
summary(mod1)
mod2 <- lm(stability~asynchronyProduction,data_tot)
summary(mod2)
mod3 <- lm(stabilityProduction~asynchronyProduction,data_tot)
summary(mod3)


# write.csv(data_tot,"data_stability.csv")
mod3 <- lm(asynchronyYield~asynchronyProduction,data_tot)
summary(mod3)
cor(data_tot$asynchronyYield,data_tot$asynchronyProduction)


dfAll <- read.csv("datasetsDerived/dataFinal_global.csv")
dfAll$synchrony <- 1-dfAll$asynchrony 
mod2 <- lm(stability~synchrony,dfAll)
summary(mod2)
